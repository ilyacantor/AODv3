...
-   **Late-Binding Domain Merge (Jan 2026):** Now enabled by default. Feature merges assets sharing the same registered domain (eTLD+1) AFTER admission. Preserves entity_id stability throughout the pipeline by applying transformation only at persistence layer. Merge rules: lens_coverage uses OR (union), lens_status uses precedence (MATCHED > AMBIGUOUS > UNMATCHED), activity_evidence uses max() timestamps. Winner selection: lexicographically smallest asset_id. Domain ordering: [canonical] + winner's domains + others (sorted). Resolves KEY_NORMALIZATION_MISMATCH for zombie detection.
