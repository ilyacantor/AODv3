...
*   **Domain Normalization Split**: Separates canonical key generation from fuzzy matching. `normalize_canonical_key()` preserves dots for storage/reconciliation (e.g., "yammer.com" stays "yammer.com"), while `get_normalization_token()` strips TLDs for fuzzy matching (e.g., "yammer" for matching).
*   **Multi-Source Corroboration**: Discovery admission now accepts assets with 3+ distinct sources from the same plane (e.g., dns + proxy + network_scan all from "network" plane). This prevents false negatives for assets heavily observed through network monitoring but not yet visible in endpoint/cloud/idp planes. Both the legacy admission code AND the Policy Engine now support this rule.
*   **Policy Engine Alignment**: Fixed critical issue where Policy Engine could override legacy admission decisions. Both systems now use identical multi-source corroboration logic (`MULTI_SOURCE_THRESHOLD = 3`). The Policy Engine's `_check_discovery_gate()` and `_build_policy_asset_data()` were updated to pass `discovery_max_sources_per_plane` for proper evaluation.
*   **Vendor Platform Domain Exclusion**: Major vendor/platform domains (google.com, microsoft.com, amazon.com, etc.) are excluded from zombie classification. These are platform providers, not individually managed enterprise assets.
