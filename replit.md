...
-   **Multi-Domain Identifiers (Jan 2026):** Assets now include ALL domains from correlated plane records (IdP, CMDB, etc.) in their identifiers.domains list. This enables Farm reconciliation to match assets by any domain variant, resolving KEY_NORMALIZATION_MISMATCH for zombie detection. **Domain Extraction Parity Fix (Jan 2026):** Domain extraction from plane records now mirrors the fallback chain used during indexing: domain → raw_data['domain'] → raw_data['external_ref'] → raw_data['url'] → raw_data['application_url'] → raw_data['service_url']. Both raw and registered domain forms are included in identifiers.domains for maximum alias matching coverage.
