...
-   **Alias Keys Expansion (Jan 2026):** Reconciliation output (`emit_actual_results`) now builds `alias_keys` from ALL domain variants including `identifiers.domains`, `all_domain_variants`, and `registered_domain`. Both `asset_details.alias_keys` and `evidence_summary.alias_keys` are populated with the expanded list. This enables Farm to look up AOD assets by any domain variant, not just the primary asset key, resolving KEY_NORMALIZATION_MISMATCH where AOD has domain evidence but doesn't use it as the canonical key.
