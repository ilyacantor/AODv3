...
-   **Late-Binding Domain Merge (Jan 2026):** Feature-flagged capability (`late_binding_domain_merge`) that merges assets sharing the same registered domain (eTLD+1) AFTER admission. Preserves entity_id stability throughout the pipeline by applying transformation only at persistence layer. Merge rules: lens_coverage uses OR (union), lens_status uses precedence (MATCHED > AMBIGUOUS > UNMATCHED), activity_evidence uses max() timestamps. Winner selection: lexicographically smallest asset_id. Domain ordering: [canonical] + winner's domains + others (sorted). Expected improvement: Zombie accuracy from ~54% to ~100% by eliminating KEY_NORMALIZATION_MISMATCH.
...
-   **Canonical Key Stability (Jan 2026):** Added `canonical_key` field to Asset model that locks the reconciliation key at admission time. Key changes: (1) registered_domain (eTLD+1) is placed FIRST in identifiers.domains instead of raw FQDN, preventing KEY_NORMALIZATION_MISMATCH, (2) canonical_key is set once during admission and validated to never change, (3) reconciliation uses asset.canonical_key when available for consistent key selection. The canonical_key field is immutable with a no-override invariant enforced at runtime.
-   **Admission Guardrail: Single-Source Rejection (Jan 2026):** Fixed a leak where single-source assets could be admitted via finance+governance paths. All admission paths except cloud now require `has_sufficient_discovery` (≥2 distinct discovery sources). Finance admission requires: (finance+idp+≥2 sources) OR (finance+cmdb+≥2 sources) OR (finance+discovery_admitted). This blocks vendor-inferred single-source assets from cluttering the catalog.
