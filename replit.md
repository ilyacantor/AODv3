...
*   **Multi-Source Corroboration**: Discovery admission now accepts assets with 3+ distinct sources from the same plane (e.g., dns + proxy + network_scan all from "network" plane). This prevents false negatives for assets heavily observed through network monitoring but not yet visible in endpoint/cloud/idp planes.
