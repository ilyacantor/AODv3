...
8. **Governance Admission Gate (Dec 2025)**: Policy engine updated to require discovery corroboration for IdP/CMDB admission. CMDB/IdP presence alone is NOT sufficient - assets must also have `discovery_planes_count >= noise_floor` (default: 2). Cloud can still admit independently. This eliminates 51 false positives where governance-only assets with insufficient discovery evidence were incorrectly admitted. Failure reason codes (e.g., `NO_DISCOVERY_CORROBORATION`) are now propagated for explainability.
