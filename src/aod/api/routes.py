...
class ShadowClassificationTraceRequest(BaseModel):
    """Request for shadow classification trace debugging"""
    run_id: str
    asset_key: str


class ShadowClassificationTraceResponse(BaseModel):
    """Response for shadow classification trace - helps debug UNGOVERNED_ACTIVE misses"""
    asset_key: str
    found: bool
    diagnosis: str
    ...
    has_discovery_evidence: bool = False
    discovery_sources: list[str] = []
    discovery_observed_at: Optional[str] = None
    
    has_idp_match: bool = False
    idp_last_login_at: Optional[str] = None
    
    has_cmdb_match: bool = False
    
    has_recent_activity: bool = False
    activity_window_days: int = 90
    latest_activity_at: Optional[str] = None
    activity_timestamp_source: Optional[str] = None
    
    shadow_rule_fired: bool = False
    shadow_blocking_reason: Optional[str] = None
    
    is_reconciliation_eligible: bool = False
    eligibility_reason: Optional[str] = None
    
    final_asset_key: Optional[str] = None
    key_source: Optional[str] = None
    
    in_shadow_actual: bool = False
    in_zombie_actual: bool = False
    
    lens_status: Optional[dict] = None
    lens_coverage: Optional[dict] = None
    evidence_refs: list[str] = []


@router.post("/debug/trace-shadow-classification")
async def trace_shadow_classification(request: ShadowClassificationTraceRequest) -> ShadowClassificationTraceResponse:
    """
    Debug endpoint: Trace why an asset is or isn't classified as shadow.
    
    Uses the ACTUAL classification pipeline (classify_actual) to show:
    - (A) Admission status and reason
    - (B) Shadow classification rule evaluation (via classify_actual)
    - (C) Final output inclusion
    """
    from ..pipeline.aod_agent_reconcile import (
        classify_actual, is_reconciliation_eligible, ReasonCode
    )
    from ..models.output_contracts import (
        Asset, AssetIdentifiers, LensStatuses, LensCoverage, 
        ActivityEvidence as ActivityEvidenceModel, LensStatus, AssetType
    )
    import uuid as uuid_lib
    ...
    asset_row = await db.fetchrow(
        """SELECT * FROM assets 
           WHERE run_id = $1 
           AND (name ILIKE $2 OR asset_id::text ILIKE $2)
           LIMIT 1""",
        request.run_id, f"%{request.asset_key}%"
    )
    ...
    if not asset_row:
        asset_row = await db.fetchrow(
            """SELECT * FROM assets 
               WHERE run_id = $1 
               AND identifiers::text ILIKE $2
               LIMIT 1""",
            request.run_id, f"%{request.asset_key}%"
        )
    
    if not asset_row:
        return ShadowClassificationTraceResponse(
            asset_key=request.asset_key,
            found=False,
            diagnosis="(A) NOT ADMITTED - Asset not found in assets table. Check if it was rejected during admission.",
            admitted=False
        )
    
    asset_data = dict(asset_row)
    
    identifiers = asset_data.get("identifiers") or {}
    if isinstance(identifiers, str):
        import json as json_lib
        identifiers = json_lib.loads(identifiers)
    
    activity_evidence = asset_data.get("activity_evidence") or {}
    if isinstance(activity_evidence, str):
        import json as json_lib
        activity_evidence = json_lib.loads(activity_evidence)
    
    lens_status_data = asset_data.get("lens_status") or {}
    if isinstance(lens_status_data, str):
        import json as json_lib
        lens_status_data = json_lib.loads(lens_status_data)
    
    lens_coverage_data = asset_data.get("lens_coverage") or {}
    if isinstance(lens_coverage_data, str):
        import json as json_lib
        lens_coverage_data = json_lib.loads(lens_coverage_data)
    
    evidence_refs_raw = asset_data.get("evidence_refs") or []
    if isinstance(evidence_refs_raw, str):
        import json as json_lib
        evidence_refs_raw = json_lib.loads(evidence_refs_raw)
    
    try:
        asset_id = uuid_lib.UUID(str(asset_data.get("asset_id")))
    except:
        asset_id = uuid_lib.uuid4()
    ...
    asset = Asset(
        ...
        run_id=asset_data.get("run_id", ""),
        name=asset_data.get("name", ""),
        asset_type=AssetType(asset_data.get("asset_type", "SAAS")),
        identifiers=AssetIdentifiers(
            domains=identifiers.get("domains", []),
            ips=identifiers.get("ips", []),
            hostnames=identifiers.get("hostnames", []),
            emails=identifiers.get("emails", [])
        ),
        vendor=asset_data.get("vendor"),
        lens_status=LensStatuses(
            idp=LensStatus(lens_status_data.get("idp", "UNMATCHED")),
            cmdb=LensStatus(lens_status_data.get("cmdb", "UNMATCHED")),
            cloud=LensStatus(lens_status_data.get("cloud", "UNMATCHED")),
            finance=LensStatus(lens_status_data.get("finance", "UNMATCHED"))
        ),
        lens_coverage=LensCoverage(
            idp=lens_coverage_data.get("idp", False),
            cmdb=lens_coverage_data.get("cmdb", False),
            cloud=lens_coverage_data.get("cloud", False),
            finance=lens_coverage_data.get("finance", False),
            discovery=lens_coverage_data.get("discovery", False)
        ),
        activity_evidence=ActivityEvidenceModel(
            discovery_observed_at=activity_evidence.get("discovery_observed_at"),
            idp_last_login_at=activity_evidence.get("idp_last_login_at"),
            finance_last_transaction_at=activity_evidence.get("finance_last_transaction_at"),
            latest_activity_at=activity_evidence.get("latest_activity_at")
        ),
        evidence_refs=evidence_refs_raw,
        admission_reason=asset_data.get("admission_reason")
    )
    
    result = classify_actual(asset, activity_window_days=90, mode="sprawl")
    
    is_eligible = is_reconciliation_eligible(asset, mode="sprawl")
    
    has_discovery = ReasonCode.HAS_DISCOVERY in result.reasons
    has_idp = ReasonCode.HAS_IDP in result.reasons
    has_cmdb = ReasonCode.HAS_CMDB in result.reasons
    has_recent_activity = ReasonCode.RECENT_ACTIVITY in result.reasons
    
    discovery_sources = [str(ref).split(":")[1] if ":" in str(ref) else str(ref) 
                        for ref in evidence_refs_raw if str(ref).startswith("discovery:")]
    
    shadow_blocking_reason = None
    if not is_eligible:
        shadow_blocking_reason = "NOT_RECONCILIATION_ELIGIBLE"
    elif has_idp or has_cmdb:
        shadow_blocking_reason = f"GOVERNED: has_idp={has_idp}, has_cmdb={has_cmdb}"
    elif not has_discovery:
        shadow_blocking_reason = "NO_DISCOVERY: No discovery evidence found"
    elif not has_recent_activity:
        shadow_blocking_reason = f"STALE: No recent activity"
    
    if result.is_shadow:
        diagnosis = "(B) CLASSIFIED AS SHADOW - Asset meets all shadow criteria via classify_actual."
    elif result.is_zombie:
        diagnosis = "(B) CLASSIFIED AS ZOMBIE - Asset meets zombie criteria (governed but stale)."
    elif not is_eligible:
        diagnosis = "(B) NOT CLASSIFIED - Not reconciliation eligible (no domain or excluded asset type)."
    else:
        diagnosis = f"(B) NOT CLASSIFIED - Shadow rule did not fire: {shadow_blocking_reason}"
    ...
    return ShadowClassificationTraceResponse(
        ...
        admitted=True,
        admission_reason=asset_data.get("admission_reason"),
        has_discovery_evidence=has_discovery,
        discovery_sources=discovery_sources,
        discovery_observed_at=str(activity_evidence.get("discovery_observed_at")) if activity_evidence.get("discovery_observed_at") else None,
        has_idp_match=has_idp,
        idp_last_login_at=str(activity_evidence.get("idp_last_login_at")) if activity_evidence.get("idp_last_login_at") else None,
        has_cmdb_match=has_cmdb,
        has_recent_activity=has_recent_activity,
        activity_window_days=90,
        latest_activity_at=str(activity_evidence.get("latest_activity_at")) if activity_evidence.get("latest_activity_at") else None,
        activity_timestamp_source=result.evidence_summary.get("activity_source"),
        shadow_rule_fired=result.is_shadow,
        shadow_blocking_reason=shadow_blocking_reason,
        is_reconciliation_eligible=is_eligible,
        eligibility_reason="Eligible" if is_eligible else "Not eligible",
        final_asset_key=result.asset_key,
        key_source=result.evidence_summary.get("key_source"),
        in_shadow_actual=result.is_shadow,
        in_zombie_actual=result.is_zombie,
        lens_status=lens_status_data,
        lens_coverage=lens_coverage_data,
        evidence_refs=evidence_refs_raw
    )


class RegressionGateRequest(BaseModel):
    """Request for regression gate validation"""
    reconcile_result: dict
    baseline_snapshot_id: Optional[str] = None


class RegressionGateResponse(BaseModel):
    """Response for regression gate validation"""
    passed: bool
    failures: list[str]
    warnings: list[str]
    current_matched: int
    current_missed: int
    current_key_mismatches: int
    matched_delta: int
    missed_delta: int
    key_mismatch_delta: int
    report: str


@router.post("/debug/regression-gate")
async def run_regression_gate(request: RegressionGateRequest) -> RegressionGateResponse:
    """
    Run regression gate validation on reconciliation result.
    ...
    3. Matched/missed count changes generate warnings
    
    Pass baseline_snapshot_id to compare against a stored baseline.
    """
    baseline = None
    
    if request.baseline_snapshot_id:
        pass
    
    result = validate_reconciliation_regression(
        current_result=request.reconcile_result,
        baseline=baseline
    )
    ...
    return RegressionGateResponse(
        ...
        current_matched=result.current_matched,
        current_missed=result.current_missed,
        current_key_mismatches=result.current_key_mismatches,
        matched_delta=result.matched_delta,
        missed_delta=result.missed_delta,
        key_mismatch_delta=result.key_mismatch_delta,
        report=report
    )
