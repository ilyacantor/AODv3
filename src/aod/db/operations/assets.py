...
Extracted from Database class - lines 514-636 and 949-1006 of original database_old.py.
"""
...
from ..serializers import _deserialize_asset_row


async def create_asset(pool, asset: Asset) -> Asset:
    """Create a new asset"""
...
async def get_assets_by_run(pool, run_id: str) -> list[Asset]:
...
async def get_asset_by_id(pool, asset_id: str) -> Optional[Asset]:
...
async def update_asset_owner(pool, asset_id: str, owner: str) -> bool:
...
async def create_assets_batch(pool, assets: list[Asset]) -> None:
    """Batch insert assets"""
    if not assets:
        return
    rows = []
